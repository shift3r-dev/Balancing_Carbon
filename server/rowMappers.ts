...
export const mapEnergyRecord = (r: any) => ({
  ...
  unit: r.unit ?? '',
  inputQuantity: Number(r.input_quantity ?? r.quantity ?? 0), inputUnit: r.input_unit ?? r.unit ?? '',
  canonicalQuantity: Number(r.canonical_quantity ?? r.quantity ?? 0), canonicalUnit: r.canonical_unit ?? r.unit ?? '', conversionFactor: Number(r.conversion_factor ?? 1), conversionPath: r.conversion_path ?? [],
  scope: r.scope ?? 'scope-1',
  emissionFactorId: r.registry_emission_factor_id ?? r.emission_factor_id ?? r.audit_trail?.emissionFactorId ?? '',
