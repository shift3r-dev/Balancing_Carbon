...
export function createMetadataRouter() {
  ...
  router.get('/metadata/render', requireAuth, requirePermission('metadata.read'), async (req: AuthenticatedRequest, res) => { try { const entityKey = str(req.query.entity); const recordId = str(req.query.recordId) || 'new'; const form = await loadMetadataForm(req.authorization!.organisationId, { formId: str(req.query.formId) || undefined, entityKey, languageCode: str(req.query.language) || 'en', publishedOnly: true }); if (!form) return res.status(404).json({ error: 'No published metadata form is configured for this entity.' }); const values = recordId === 'new' ? {} : await existingValuesByRender(req.authorization!.organisationId, form.entityKey, recordId, form); const rendered = renderMetadataForm(form, values, { permissions: req.authorization!.permissions, roleIds: req.authorization!.roles }); res.json({ form: rendered, values }); } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Unable to render metadata form.' }); } });
  router.post('/metadata/render/:entityKey/:recordId', requireAuth, requirePermission('metadata.values.edit'), async (req: AuthenticatedRequest, res) => { try { const form = await loadMetadataForm(req.authorization!.organisationId, { formId: str(req.body?.formId) || undefined, entityKey: req.params.entityKey, publishedOnly: true }); if (!form) return res.status(404).json({ error: 'No published metadata form is configured for this entity.' }); const result = await saveMetadataValues({ organisationId: req.authorization!.organisationId, userId: req.authUser!.id, entityKey: req.params.entityKey, entityRecordId: req.params.recordId, form, values: req.body?.values ?? {}, context: { permissions: req.authorization!.permissions, roleIds: req.authorization!.roles } }); if (result.issues.length) return res.status(422).json(result); res.json(result); } catch (error) { res.status(400).json({ error: error instanceof Error ? error.message : 'Unable to save metadata values.' }); } });
