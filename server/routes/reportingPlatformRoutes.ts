...
export function createReportingPlatformRouter() {
  ...
  router.post('/reporting/reports/generate', requireAuth, requireOperationalLicense, requirePermission('report.generate'), requireEntitlement('reports.generate'), requireLimit('reports_month', monthlyReportUsage), async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const [totalDisplay, scope1Display, scope2Display, scope3Display] = await Promise.all([
        ...
        smartDisplay({ value: snapshot.scope3Tco2e * 1000, unit: 'kgCO2e', measurementSystem: localization.measurementSystem, preferredUnit: preferredCarbonUnit }),
      ]);
      snapshot.display = { total: { value: totalDisplay.displayValue, unit: totalDisplay.displayUnit }, scope1: { value: scope1Display.displayValue, unit: scope1Display.displayUnit }, scope2: { value: scope2Display.displayValue, unit: scope2Display.displayUnit }, scope3: { value: scope3Display.displayValue, unit: scope3Display.displayUnit } };
