...
const provider = () => new OllamaProvider(runtimeConfig.ai.baseUrl, runtimeConfig.ai.model, runtimeConfig.ai.contextLength, runtimeConfig.ai.timeoutMs);
...
async function completeChat(input: { organisationId: string; userId: string; question: string; conversation: any; signal?: AbortSignal }) {
  ...
  const row = { id: conversationId, organisation_id: input.organisationId, created_by: input.userId, title: input.conversation?.title ?? input.question.slice(0, 80), provider: 'ollama', model: runtimeConfig.ai.model, messages: nextMessages, last_updated: now };
  const { error } = await supabaseAdmin.from('ai_conversations').upsert(row); if (error) throw new Error(error.message);
  await supabaseAdmin.from('ai_usage_events').insert({ id: `ai-usage-${randomUUID()}`, organisation_id: input.organisationId, user_id: input.userId, conversation_id: conversationId, provider: 'ollama', model: runtimeConfig.ai.model, status: 'completed', prompt_tokens: result.promptTokens ?? null, completion_tokens: result.completionTokens ?? null, duration_ms: result.durationMs, metadata: { citationCount: usedCitations.length } });
  return { conversationId, messages: nextMessages, provider: 'ollama', model: runtimeConfig.ai.model, usage: { durationMs: result.durationMs, promptTokens: result.promptTokens, completionTokens: result.completionTokens } };
}

async function recordFailure(organisationId: string, userId: string, conversationId: string | null, error: unknown) {
  await supabaseAdmin.from('ai_usage_events').insert({ id: `ai-usage-${randomUUID()}`, organisation_id: organisationId, user_id: userId, conversation_id: conversationId, provider: 'ollama', model: runtimeConfig.ai.model, status: 'failed', duration_ms: 0, error_code: error instanceof Error && error.name === 'AbortError' ? 'cancelled_or_timeout' : 'provider_error' });
}

export function createAiCopilotRouter() {
  const router = Router();
  router.get('/ai/status', requireAuth, async (_req, res) => {
    if (!runtimeConfig.ai.enabled) return res.json({ enabled: false, provider: runtimeConfig.ai.provider, available: false });
    const health = await provider().health(); res.json({ enabled: true, provider: 'ollama', model: runtimeConfig.ai.model, localOnly: !runtimeConfig.ai.allowExternal, ...health });
  });
  ...
  router.post('/ai/chat', requireAuth, requirePermission('ai.use'), async (req: AuthenticatedRequest, res) => {
    ...
    if (!runtimeConfig.ai.enabled || runtimeConfig.ai.provider !== 'ollama') return res.status(503).json({ error: 'Local AI is disabled.' });
    ...
    try {
      ...
      res.status(503).json({ error: error instanceof Error && error.name === 'AbortError' ? 'Local model timed out.' : (error instanceof Error ? error.message : 'Local model request failed.') });
    }
  });
  router.post('/ai/chat/stream', requireAuth, requirePermission('ai.use'), async (req: AuthenticatedRequest, res) => {
    ...
    try { emit('context', { message: 'Loading tenant records and citations.' }); emit('generating', { message: `Generating locally with ${runtimeConfig.ai.model}.` }); const result = await completeChat({ organisationId, userId, question, conversation, signal: controller.signal }); emit('complete', result); res.end(); }
    catch (error) { await recordFailure(organisationId, userId, conversation?.id ?? null, error); emit('error', { error: error instanceof Error && error.name === 'AbortError' ? 'Generation cancelled or timed out.' : (error instanceof Error ? error.message : 'Local model request failed.') }); res.end(); }
  });
  return router;
}
