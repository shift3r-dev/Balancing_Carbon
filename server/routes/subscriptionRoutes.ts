...
export function createSubscriptionRouter() {
  ...
  router.post('/subscription/upgrade', requireAuth, requirePermission('subscription.manage'), async (req: AuthenticatedRequest, res) => { try { res.json({ subscription: await changeSubscription({ organisationId: req.authorization!.organisationId, userId: req.authUser!.id, planId: req.body?.planId ?? req.body?.plan_id, action: 'upgrade' }) }); } catch (e) { res.status(400).json({ error: e instanceof Error ? e.message : 'Failed to change subscription.' }); } });
