...
-- Pricing remains intentionally unconfigured until commercial prices are approved.
...
cross join (values ('plan-professional'), ('plan-enterprise'), ('plan-enterprise-plus')) as target(plan_id)
...
from plan_features source
cross join (values ('plan-enterprise'), ('plan-enterprise-plus')) as target(plan_id)
where source.plan_id = 'plan-professional'
on conflict (plan_id, feature_key) do update set label=excluded.label, category=excluded.category, availability=excluded.availability, sort_order=excluded.sort_order;

insert into plan_features (id, plan_id, feature_key, label, category, availability, sort_order)
select 'feature-plan-enterprise-plus-' || source.feature_key,
       'plan-enterprise-plus', source.feature_key, source.label, source.category, source.availability, source.sort_order + 200
from plan_features source
where source.plan_id = 'plan-enterprise'
on conflict (plan_id, feature_key) do update set label=excluded.label, category=excluded.category, availability=excluded.availability, sort_order=excluded.sort_order;
